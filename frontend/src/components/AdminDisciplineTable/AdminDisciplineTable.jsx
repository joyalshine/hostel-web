...
import "./AdminDisciplineTable.css";
    ...
    if (cookieData.length !== 0) {
      ...
      response.map((item, index) => {
        timeModifiedData.push({
          ...item,
          timestamp: new Date(item.createdAt).toString(),
        });
      });
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <div className="alert">no complains</div>
      ) : (
...
function TableRow({ item, index, setComplaint }) {
  ...
  const handleDeny = async () => {
    if (feedback.length === 0 || feedback === null) {
      ...
      await setFeedBacktoDB("discipline", feedback, userId, item);
      setComplaint((prev) =>
        prev.filter((element) => {
...
export default AdminDisciplineTable;
