...
import { CLEANING, BLOCKS } from "../../dataAssets";
...
const CleaningTable = () => {
  ...
  const [cookies, setCookie] = useCookies(["cleaning_cookie"]);

  const fetchData = async () => {
    setLoading(true);
    let cookieExists = cookies.cleaning_cookie;
    ...
    if (cookieData.length !== 0) {
      ...
      const response = await getComplaints("cleaning");
      ...
      //     room: item.room,
      //     complainDesc: item.complainDesc,
        ...
        <div class="filters row justify-content-md-center p-3">
          ...
          <div class="col">
            ...
            >
              ...
              {Object.keys(CLEANING).map((key) => {
                return <option value={key}>{CLEANING[key]}</option>;
              })}
            </select>
          </div>
          <div class="col">
            <button type="submit" class="btn-1">
              <CSVLink
                style={{ color: "white" }}
                filename={`PENDING-CLEANING-COMPLAIN-${format(
                  new Date(),
                  "dd-MM-yyyy"
                )}`}
                data={downloaData}
...
function TableRow({ item, index, setComplaint }) {
  ...
  let forData = CLEANING[item.category[0]];
  ...
  const handleDeny = async () => {
    if (feedback.length === 0 || feedback === null) {
      ...
      await setFeedBacktoDB("cleaning", item._id, feedback, userId, item);
      setComplaint((prev) =>
        prev.filter((element) => {
  ...
  const handleReslove = async () => {
    const response = await upadateComplaint("cleaning", item._id, userId, item);
    setComplaint((prev) =>
      prev.filter((element) => {
        return item._id !== element._id;
      })
